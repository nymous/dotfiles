...
20. Enable `systemd-boot-update.service`, and create a post-install hook to automatically update systemd-boot in the ESP when the package is upgraded: `/etc/pacman.d/hooks/100-systemd-boot.hook`
    ...
    ```
21. Install CPU microcode: `pacman -Sy amd-ucode`
22. Make sure the `microcode` hook is configured in `/etc/mkinitcpio.conf`
23. Create a new boot entry (to insert the UUID in the vim file, run `:r!blkid -o value /dev/<whatever> | head -n1`): `/boot/loader/entries/arch.conf`
    ```
    title   Arch Linux
    linux   /vmlinuz-linux
    # microcode is packaged in initramfs using mkinitcpio hooks
...
24. Configure systemd-boot: `/boot/loader/loader.conf`
    ```
    timeout 3
    # From loader.conf(5): the file name including literal suffix ".conf"
    default arch.conf
    ```
...
7. Enable FreeSync: create `/etc/X11/xorg.conf.d/10-freesync.conf` with the following content:
    ...
    EndSection
    ```
8. Configure graphical session env variables: create `/etc/X11/Xsession.d/90-environment`
    ```
    export XDG_CURRENT_DESKTOP="i3"
    export XDG_SESSION_DESKTOP="i3"
    ```
9. Set Xorg keymap: `sudo localectl set-x11-keymap fr pc105 oss_latin9 ctrl:swapcaps,compose:rctrl-altgr` (the `oss_latin9` variant gives you easy access to "œ", "æ" or "…" characters, the `compose:rctrl-altgr` option enables the [compose key](https://wiki.archlinux.org/title/Xorg/Keyboard_configuration#Configuring_compose_key) for any other character you would need (keep <kbd>AltGr</kbd> pressed then press <kbd>Right Ctrl</kbd>), and the `ctrl:swapcaps` swaps the position of <kbd>Ctrl</kbd> (often used for shortcuts but stuck in the corner) with <kbd>Caps lock</kbd> (where the pinky naturally sits))
10. Install `numlockx`
11. Install `i3` and choose `i3-gaps` over `i3-wm`
12. Install a terminal, eg. `alacritty` (with `alacritty-terminfo`)
13. Install `xorg-xinit`, copy the configuration file from `/etc/X11/xinit/xinitrc` to `~/.xinitrc`, then edit the last lines with the following:
    ```shell
    numlockx &
    exec i3
    ```
14. Since version 1.44 Pango doesn't support bitmap fonts anymore, so you need to install an OTF/TTF font such as Fira Code: `otf-fira-code` (you can remove `xorg-fonts-{75,100}dpi`)
15. You can now finally start the X server! `startx` \o/
16. For hardware decoding and encoding you can install `libva-mesa-driver` and `mesa-vdpau`, and check that everything works with `vainfo` and `vdpauinfo` (available in the packages `libva-utils` and `vdpauinfo`)
17. Now that you have a graphical interface you should install a browser, so that you can copy-paste commands instead of retyping them from another computer ^^' `yay -Sy firefox-nightly`
18. Install the launcher (you need to run Firefox at some point): `yay -Sy rofi` and add a keybind in the i3 config: `bindsym $mod+d exec rofi -show drun -matching fuzzy -show-icons`
19. Create your SSH key: `ssh-keygen -o -a 100 -t ed25519 -C "<user>@<hostname>"` (from <https://medium.com/risan/upgrade-your-ssh-key-to-ed25519-c6e8d60d3c54>)
20. Clone this repository: `git clone git@github.com:nymous/dotfiles.git`
21. Install [twpayne/chezmoi](https://github.com/twpayne/chezmoi): `yay -Sy chezmoi`
22. Run `chezmoi diff` to see what will be updated, then `chezmoi apply -v` to apply all dotfiles
23. For `pass` you need to copy your key in the `gitolite-admin` repo, give it the correct rights then clone the pass repo. You then need to import the GPG key using `gpg --import key.asc`.
